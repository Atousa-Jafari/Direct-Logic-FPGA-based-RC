 ...
 */
#define MAX_TRANSFER_PKT_LEN		0x1388
#define MAX_RECEIVE_PKT_LEN		0x4E20
...
xil_printf("Receiver address = %08x\r\n", RX_BUFFER_BASE);    
    ...
    memcpy((void *)TxBufferPtr, (void *)data, MAX_TRANSFER_PKT_LEN);

	/* Flush the buffers before the DMA transfer, in case the Data Cache
	 * is enabled
	 */
	Xil_DCacheFlushRange((UINTPTR)TxBufferPtr, MAX_TRANSFER_PKT_LEN);
	Xil_DCacheFlushRange((UINTPTR)RxBufferPtr, MAX_RECEIVE_PKT_LEN);

	/* Send a packet */
	for (Index = 0; Index < Tries; Index ++) {

		Status = XAxiDma_SimpleTransfer(&AxiDma, (UINTPTR) RxBufferPtr,
						MAX_RECEIVE_PKT_LEN, XAXIDMA_DEVICE_TO_DMA);
		...
		Status = XAxiDma_SimpleTransfer(&AxiDma, (UINTPTR) TxBufferPtr,
						MAX_TRANSFER_PKT_LEN, XAXIDMA_DMA_TO_DEVICE);
		 ...
		 */
		// Status = CheckData(MAX_RECEIVE_PKT_LEN);
		// if (Status != XST_SUCCESS) {
